...
## -*- texinfo -*- 
## @deftypefn {} {@var{retval} =} spider_plot (@var{input1}, @var{input2})
##
## @seealso{}
## @end deftypefn
## Author: mauro <mauro@GOLDA-19610808>
## Created: 2020-04-20
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Grafico de SpiderPlot - radar
% Função spider_plot()
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if numvarargs > 1
    ...
    for ii = 1:length(name_arguments)
        ...
        end #switch lower(name_arguments{ii})
    end #for ii = 1:length(name_arguments)
...
if ~isempty(axes_limits)
    ...
    end #if size(axes_limits, 1) ~= 2 || size(axes_limits, 2) ~= num_data_points
...
end #if floor(axes_interval) ~= axes_interval || floor(axes_precision) ~= axes_precision
...
end #if axes_interval < 1 || axes_precision < 0
...
end #if ~ismember(axes_display, {'all', 'none', 'one'})
...
end #if ~ismember(fill_option, {'off', 'on'})
...
end #if fill_transparency < 0 || fill_transparency > 1
...
end #if axes_font_size <= 0 || label_font_size <= 0
...
end #if ~ismember(direction, {'counterclockwise', 'clockwise'})
...
end #if ~ismember(axes_scaling, {'linear', 'log'})
...
end #if num_data_groups == 1 && isempty(axes_limits)
...
elseif iscellstr(marker_type)
    ...
    elseif length(marker_type) ~= num_data_groups
        error('Error: Please specify the same number of line styles as number of data groups.');
    end #if length(marker_type) == 1
else
    error('Error: Please make sure the line style is a char or a cell array of char.');
end #if ischar(marker_type)
% Check if line width is numeric
if isnumeric(marker_size)
    ...
    elseif length(marker_size) ~= num_data_groups
        error('Error: Please specify the same number of line width as number of data groups.');
    end #if length(marker_size) == 1
else
    error('Error: Please make sure the line width is numeric.');
end #if isnumeric(marker_size)
...
end #if strcmp(axes_scaling, 'log')
%%% Figure Properties %%%
% Grab current figure
fig = gcf;
...
for ii = 1:num_data_points
    ...
    if isempty(axes_limits)
        ...
        end #if min_value > min(group_points) || max_value < max(group_points)
...
for ii = 1:theta_end_index
    ...
    end #for jj = 2:length(rho)
end #for ii = 1:theta_end_index
%%% Plot %%%
...
for ii = 1:num_data_groups
    ...
    if strcmp(fill_option, 'on')
        ...
        % Turn off legend annotation
        h.Annotation.LegendInformation.IconDisplayStyle = 'off';
    end #if strcmp(fill_option, 'on')
end #for ii = 1:num_data_groups
% Find object handles
...
if ~strcmp(axes_labels, 'none')
    ...
    end #for ii = 1:length(axes_labels)
end #if ~strcmp(axes_labels, 'none')
endfunction #function spider_plot (P, varargin)